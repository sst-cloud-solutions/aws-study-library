...
:::note
**Real-World Analogy:** Searching for a name in a massive stack of tax papers: instead of scanning them manually, you use a magic scanner that searches all sheets in the boxes instantly and highlights the matches.
:::
...
| Parameter | Amazon Athena | Amazon Redshift |
| :--- | :--- | :--- |
...
:::tip
**Exam Clues:** athena, serverless sql query, query s3 data lake, parquet compression, scan partition search

Use Athena to query server log streams (ALB logs, CloudTrail logs, VPC Flow Logs) stored in S3.
:::

:::warning
**Common Exam Traps:** Do not use Athena for highly transactional database applications (OLTP) due to query latency overhead.
:::
...
- [AWS Lake Formation](../Data Integration & Management/AWS Lake Formation.md)
